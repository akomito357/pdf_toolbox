...
def convert(convert_list):
    if convert_list != []:
        for pic in convert_list:
            im = Image.open(pic)
            im.save(pic[:-3]+'pdf')
        convert_msg = convert_list, '已被轉為pdf檔。'    
    elif convert_list == []:
...
descrip_title01 = tk.Label(descrip_frame, font=('微軟正黑體', 9, 'bold'),  text='1. 獲取pdf文字')
...
一次處理太多檔案可能會造成程式卡頓。\n''')
descrip_title02 = tk.Label(descrip_frame, font=('微軟正黑體', 9, 'bold'), text='2. 將圖檔轉為pdf')
...
descrip_title03 = tk.Label(descrip_frame, font=('微軟正黑體', 9, 'bold'), text='3. 合併pdf')
...
btn_get_text = tk.Button(button_frame, text='【獲取pdf文字】', command=lambda: get_text(get_file('pdf')))
btn_get_text.grid(column=0, row=0, padx=20, pady=10)

btn_convert = tk.Button(button_frame, text='【將圖檔轉為pdf】', command=lambda: convert(get_file(('jpg', 'jpeg', 'png'))))
btn_convert.grid(column=2, row=0, padx=20, pady=10)

btn_merge = tk.Button(button_frame, text='【合併pdf】', command=lambda: merge_pdf(get_file('pdf')))
